## Hi there 👋

<!--
**Abdulquddus-Nuhu/Abdulquddus-Nuhu** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->

Backend Engineer focused on fintech & banking infrastructure.

## 🚀 About Me
- Building backend systems with .NET
- Working on banking & NIBSS integrations
- Interested in scalable fintech architecture
- Exploring distributed systems & cloud infrastructure

## 🛠️ Tech Stack

<p align="left">
  <img src="https://skillicons.dev/icons?i=cs,dotnet,mysql,postgres,redis,docker,git,github,githubactions,postman,visualstudio,azure,nginx,rabbitmq,prometheus" />
</p>

[![Icoziv Skills](https://i.icoziv.workers.dev/icons?i=cs,dotnet,javascript,blazor-light,sqlserver,postgres,mysql,redis,docker,podman,git,github,githubactions,postman,visualstudio,visualstudiocode,azure,nginx,rabbitmq,grafana-light,prometheus,linux-light,debian,&perline=10)](https://github.com/thuongtruong109/icoziv)

## 📌 Current Focus
- Banking integrations
- Payment systems
- API security
- Distributed backend systems

## 📊 GitHub Stats

![GitHub stats](https://github-readme-stats.vercel.app/api?username=Abdulquddus-Nuhu&show_icons=true&theme=tokyonight)

## 🔥 Streak Stats

![GitHub Streak](https://streak-stats.demolab.com?user=Abdulquddus-Nuhu&theme=tokyonight)

## 🌐 Connect With Me

- LinkedIn: [<img src="https://skillicons.dev/icons?i=linkedin" />](https://www.linkedin.com/in/abdulquddus-nuhu-b9b59319a/)
- Twitter/X: [<img src="https://skillicons.dev/icons?i=twitter" />](https://x.com/AbdulquddusNuhu)
